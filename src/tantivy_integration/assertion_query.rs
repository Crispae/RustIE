...
/// Assertion type for sentence boundaries
#[derive(Debug, Clone)]
...
/// Query that filters matches based on sentence position assertions
#[derive(Debug)]
pub struct SentenceAssertionQuery {
    assertion_type: AssertionType,
    default_field: Field,
}

impl Clone for SentenceAssertionQuery {
    fn clone(&self) -> Self {
        SentenceAssertionQuery {
...
impl SentenceAssertionQuery {
    pub fn new(assertion_type: AssertionType, default_field: Field) -> Self {
        Self {
            assertion_type,
            default_field,
        }
...
impl Query for SentenceAssertionQuery {
    fn weight(&self, _scoring: EnableScoring<'_>) -> TantivyResult<Box<dyn Weight>> {
        Ok(Box::new(SentenceAssertionWeight {
            assertion_type: self.assertion_type.clone(),
            default_field: self.default_field,
        }))
    }
...
struct SentenceAssertionWeight {
    assertion_type: AssertionType,
    default_field: Field,
}

impl Weight for SentenceAssertionWeight {
    fn scorer(&self, reader: &SegmentReader, _boost: Score) -> TantivyResult<Box<dyn Scorer>> {
        let mut scorer = SentenceAssertionScorer {
            assertion_type: self.assertion_type.clone(),
            default_field: self.default_field,
            reader: reader.clone(),
            current_doc: None,
    ...
    fn explain(&self, _reader: &SegmentReader, _doc: DocId) -> TantivyResult<tantivy::query::Explanation> {
        Ok(tantivy::query::Explanation::new("SentenceAssertionQuery", Score::default()))
    }
}

pub struct SentenceAssertionScorer {
    assertion_type: AssertionType,
    default_field: Field,
...
impl Scorer for SentenceAssertionScorer {
    fn score(&mut self) -> Score {
        1.0
    }
}

impl DocSet for SentenceAssertionScorer {
    fn advance(&mut self) -> DocId {
        // Sentence assertions are zero-width - they match all documents
        // but the actual position filtering happens at a higher level
        // For now, we iterate through all documents
        
        if self.next_doc_id >= self.all_docs {
...
impl SentenceAssertionScorer {
    /// Get the assertion position requirement
    /// Returns (expected_position, is_from_end) 
    /// - SentenceStart: position must be 0
    /// - SentenceEnd: position must be (sentence_length - 1)
    pub fn get_position_requirement(&self) -> (usize, bool) {
        match self.assertion_type {
            AssertionType::SentenceStart => (0, false),
            AssertionType::SentenceEnd => (0, true), // true = from end
        }
    }
}
